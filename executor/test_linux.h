...
{
	printf("\n=== testing text %d, text size 0x%x, flags 0x%x\n", text_type, text_size, flags);
	...
	int vm_mem_size = 24 * SYZ_PAGE_SIZE; // Allocate what syzkaller normally allocates
...
#elif GOARCH_ppc64le
	res = test_one(8, kvm_ppc64_mr, sizeof(kvm_ppc64_mr) - 1, KVM_SETUP_PPC64_LE | KVM_SETUP_PAGING, KVM_EXIT_DEBUG, true);
	if (res)
		return res;
	res = test_one(8, kvm_ppc64_mr, sizeof(kvm_ppc64_mr) - 1, KVM_SETUP_PAGING, KVM_EXIT_DEBUG, true);
	if (res)
		return res;
	res = test_one(8, kvm_ppc64_ld, sizeof(kvm_ppc64_ld) - 1, KVM_SETUP_PPC64_LE | KVM_SETUP_PAGING, KVM_EXIT_DEBUG, true);
	if (res)
		return res;
	res = test_one(8, kvm_ppc64_ld, sizeof(kvm_ppc64_ld) - 1, KVM_SETUP_PAGING, KVM_EXIT_DEBUG, true);
	if (res)
		return res;
	res = test_one(8, kvm_ppc64_mr, sizeof(kvm_ppc64_mr) - 1, KVM_SETUP_PPC64_LE, KVM_EXIT_DEBUG, true);
	if (res)
		return res;
	res = test_one(8, kvm_ppc64_mr, sizeof(kvm_ppc64_mr) - 1, 0, KVM_EXIT_DEBUG, true);
	if (res)
		return res;
	res = test_one(8, kvm_ppc64_ld, sizeof(kvm_ppc64_ld) - 1, KVM_SETUP_PPC64_LE, KVM_EXIT_DEBUG, true);
	if (res)
		return res;
	res = test_one(8, kvm_ppc64_ld, sizeof(kvm_ppc64_ld) - 1, 0, KVM_EXIT_DEBUG, true);
	if (res)
		return res;
#endif
